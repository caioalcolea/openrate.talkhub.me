...
| 8 | **Hardening do PWA** | Web Push (VAPID) para notificações in-app além do WhatsApp, robustez da fila offline (background sync), cache de assets/versionamento do service worker, banner de instalação e teste cross-browser (Chrome Android, Safari iOS). Sem lojas de app | Melhora a experiência sem depender de revisão de store; o produto continua 100% web |
| 9 | **(Opcional) App nativo** | Só se o PWA esbarrar em limite real de plataforma (ex.: gravação em background no iOS). Entraria como `apps/mobile` reaproveitando `packages/shared`; decisão puxada por dado de uso, não por default | Evitar toolchain nativa e revisão de lojas enquanto o PWA atender |

Cada item vira uma sprint (ou meia) com o mesmo formato das anteriores quando puxado. Itens 1, 2 e 3 têm dependência apenas do que já existe; 5 depende de 3; 6 depende de contas/aprovação de developer nas plataformas (iniciar os cadastros já na Sprint 4-5, o lead time é de semanas). O item 9 é contingente — só entra se o 8 revelar um bloqueio de plataforma.
...
1. **Produto 100% web — risco de plataforma no navegador, não em lojas.** Não há submissão a App Store/Play (o atendente usa o PWA), então o gargalo de revisão de loja **deixa de existir**. O risco que resta é de capacidade do navegador: `MediaRecorder`/`getUserMedia` variam entre Chrome Android e Safari iOS, e o iOS impõe limites a PWA instalado (ex.: gravação em background, cotas de storage). Mitigação: negociar mime-type em runtime e normalizar no worker; testar cedo em iOS **e** Android reais; manter a opção de app nativo (`apps/mobile`) como item contingente 9 do backlog, só se um limite real bloquear.
